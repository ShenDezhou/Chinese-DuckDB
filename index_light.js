...
function urlMapToPath(url) {
    ...
    const matches = url.match(/https:\/\/huggingface\.co\/datasets\/(_-\w]+)\/resolve\/refs%2Fconvert%2Fparquet\/([\.-_\w]+)\/([\w-_]+\.parquet)/);
    console.log(matches)
...
function urlMapToDSCache(url) {
    ...
    const matches = url.match(/https:\/\/huggingface\.co\/datasets\/([_-\w]+)\/raw\/main\/dataset_infos.json/);
    console.log(matches)
